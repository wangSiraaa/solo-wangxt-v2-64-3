...
export class FeesService {
  ...
  async feeSegments(
    ...
    const projection = await this.dataSource.transaction((manager) =>
      this.billingProjection.project(manager, elderId, from, to, 'TRIAL'),
    );

    // 天数守恒：逐天投影必须覆盖闭区间每一天
    if (projection.totalDays !== diffDays(to, from) + 1) {
      throw new Error(
        `费用分段天数不一致：分段 ${projection.totalDays} 天，区间 ${diffDays(to, from) + 1} 天`,
      );
    }
    return projection;
  }
